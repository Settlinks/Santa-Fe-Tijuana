...
SFT.GAS_URL    = 'https://script.google.com/macros/s/AKfycby9GiKphx7LLA28jUoyg3a_k_FUiW-ELWDBANNTCEq3KScOY5HtkvIBGqcxb36jX_7okA/exec';
...
/* ════ IMAGE URL FIX ════
   The lh3.googleusercontent.com/d/{id}=w600 format (used for logos/photos
   stored on Drive) is unreliable for hotlinking and frequently fails to
   load. Rewrite ANY Google Drive file URL (old or new format) to Drive's
   official thumbnail endpoint, which works for files shared
   "Anyone with the link can view". */
function driveImgUrl(url) {
  if (!url) return '';
  const u = String(url);
  const m = u.match(/\/d\/([-\w]{15,})/);
  if (m) return 'https://drive.google.com/thumbnail?id=' + m[1] + '&sz=w600';
  return u;
}
/* ════ QR CODE FIX ════
   Google's Chart API (chart.googleapis.com/chart?cht=qr) has been
   deprecated/shut down and now returns errors. Use goqr.me's free,
   key-less QR API instead — same idea, different host. */
...
function openDetail(type, item, allItems) {
  ...
  const imgUrl = driveImgUrl(item.imageUrl || '');
  ...
  const phone = String(item.phone||item.contactPhone||'');
  ...
  if (qrWrap) {
    if (type === 'biz' && slug) {
      ...
      var qrSrc = qrCodeUrl(bizPageUrl, '200x200');
      qrWrap.innerHTML =
